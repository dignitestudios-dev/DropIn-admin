import { coverPhoto, defaultImage } from "../../../assets/export";
...
import {
  ...
  formatToDay,
  formatToFullMonthYearWithDay,
  formatToMonthAbbreviation,
} from "../../../lib/helpers";
...
export default function EventDetail() {
  ...
  const getEventDetail = async () => {
    try {
      ...
      const response = await axios.get(
        `/event/get-event-details?eventID=${event._id}`,
      );
  ...
  const getEventPeopleGoing = async () => {
    try {
      setLoading(true);
      const response = await axios.get(
        `/event/get-event-attendees?eventID=${event._id}&page=1&limit=10`,
      );
  ...
  return loadingDetail ? (
    ...
    <div>
      <div className="relative w-[100%] flex justify-center">
        <img
          src={eventDetail.coverPhoto ? eventDetail.coverPhoto : defaultImage}
          alt="LogInBg"
            ...
            <div className="flex items-center gap-3 mt-2">
              ...
              <div className="flex items-center gap-2">
                ...
                <p className="text-white font-[400] text-[11px]  ">
                  ...
                  {eventDetail.attendeeLimit && `/${eventDetail.attendeeLimit}`}
                </p>
              </div>
            </div>
          </div>
